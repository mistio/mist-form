...
updateUnitValue(e) {
  ...
  updateValue() {
    ...
    this.value = this.textValue && this.unitValue ? `${this.textValue}${this.unitValue}` : undefined;  }

  validate() {
    return !!(this.textValue && this.unitValue) || !!(!this.textValue && !this.unitValue);
  }
  ...
  render() {
    ...
    return html` <span>${this.label}</span>
      <paper-input
        ...
        max=${this.max}
        type="number"
        autovalidate="true" excludeFromPayload >
        @value-changed=${this.updateTextValue}
      ></paper-input>
      <paper-dropdown-menu excludeFromPayload>
        <paper-listbox @selected-changed=${this.updateUnitValue} class="dropdown-content" slot="dropdown-content">
          ${this.units.map(
            unit =>
              html`<paper-item
