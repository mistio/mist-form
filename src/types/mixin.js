  ...
  class extends superClass {
    ...
    get domValue() {
      ...
      if (this.widget === 'select') {
        ...
        field._menuElement.querySelectorAll('vaadin-item').forEach((i,j) => {if (i.getAttribute('value') == field.value) field._menuElement.selected = j});
      }
      return this.cast(field.value !== undefined ? field.value : field.getAttribute('value'));
    }

    get widget() {
      if (this.spec.uiSchema && this.spec.uiSchema["ui:widget"] && this.spec.uiSchema["ui:widget"]) {
    ...
    get isReadOnly() {
      return Boolean((this.spec.uiSchema && this.spec.uiSchema['ui:readonly']) || this.spec.jsonSchema.readOnly)
    }
      ...
      switch (this.widget) {
        ...
        case 'radio':
          if (customElements.get('vaadin-radio-group') === undefined) {
            import('@vaadin/radio-group/theme/material/vaadin-radio-group.js').then(console.debug('imported vaadin radio group'));
          }
          break;
        case 'number':
          if (customElements.get('vaadin-number-field') === undefined) {
            import('@vaadin/number-field').then(console.debug('imported vaadin number field'));
          }
          break;
        case 'integer':
          if (customElements.get('vaadin-integer-field') === undefined) {
            import('@vaadin/integer-field').then(console.debug('imported vaadin integer field'));
          }
          break;
        case 'range':
          break;
