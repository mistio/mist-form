describe('Constraints form with data', () => {
  ...
  it('Check that data appears correctly for expiration subform', () => {
    cy.get('mist-form')
      ...
      .within(() => {
        ...
        cy.get('#default').find('paper-input').find('input').should('have.value', 20);
        cy.testPaperDropdownSelected(
          '#default',
          'd'
        );
        ...
        cy.get('#expiration_notify').within(() => {
          ...
          cy.get('#require').should('have.attr', 'checked');
  ...
  it('Check that data appears correctly in field subform', () => {
    cy.get('mist-form')
      ...
      .within(() => {
        cy.get('input').first().should('have.value', 'Field2');
        cy.get('input').eq(1).should('have.value', 'Value2');
        cy.get('input').eq(2).should('have.value', 'Field3');
        cy.get('paper-checkbox').first().should('not.have.attr', 'checked');
        cy.get('paper-checkbox').last().should('have.attr', 'checked');
      });
  });
